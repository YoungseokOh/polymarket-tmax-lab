...
`run_recent_core_benchmark.py`는 recent `Seoul` / `NYC` / `London` benchmark를 city별 격리 디렉터리로 재실행하고, summary JSON에 `real_history`, `quote_proxy`, 그리고 `city x horizon` delta까지 같이 적는다. 기존 산출물이 있으면 `--reuse-existing`로 summary만 다시 만들 수 있다.
