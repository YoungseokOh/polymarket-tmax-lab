...
uv run pmtmax materialize-backtest-panel --dataset-path data/parquet/gold/historical_training_set.parquet --markets-path configs/market_inventory/historical_temperature_snapshots.json --allow-canonical-overwrite
...
## Synthetic NWP Single-Run Re-Collection

### 배경 (NWP Bug 2)

`historical_forecast` API는 모든 모델 이름(gfs, ecmwf, kma)에 대해 동일한 ERA5
재분석값을 반환한다. 이 때문에 학습 데이터의 NWP 다양성(nwp_spread)이 0이 되는
구조적 문제가 있다. 이를 해결하기 위해 `single_run` API(archived NWP run)로
103,110개의 synthetic 마켓 데이터를 재수집해야 한다.

### 올바른 명령

```bash
nohup scripts/pmtmax-workspace research_synth uv run pmtmax backfill-forecasts \
  ...
  --missing-only \
  > artifacts/batch_logs/single_run_$(date +%Y%m%dT%H%M%S).log 2>&1 &
```
...
| 파일 | 마켓 수 | 용도 |
|------|---------|------|
| `synthetic_historical_snapshots.json` | 103,110 | ✅ **NWP 재수집 전용** |
| `synthetic_gfs_eligible.json` | 42,870 | GFS 2021-07+ 서브셋 (불완전) |
| `synthetic_ready_snapshots.json` | 91,673 | truth+forecast 준비 서브셋 |
| `full_training_set_snapshots.json` | 1,834 | 실제(real) 마켓 — NWP 재수집에 사용 금지 |

`full_training_set_snapshots.json`은 1,834개의 실제 Polymarket 마켓이다.
synthetic 재수집에 쓰면 103k synthetic이 아닌 1.8k real 마켓만 처리되는 치명적 실수다.

### 모델별 archive 시작일 (`SINGLE_RUN_MODEL_MIN_DATE`)

코드(`src/pmtmax/backfill/pipeline.py`)에 하드코딩된 값으로, 이 날짜 이전 요청은
API 호출 없이 즉시 스킵된다.

| 모델 | archive 시작 | 영향 범위 |
|------|-------------|---------|
| `gfs_seamless` | 2021-07-01 | 학습셋 ~44% (2021-07~) |
| `ecmwf_ifs025` | 2024-03-01 | 학습셋 ~14% (2024-03~) |
| `kma_gdps` | 2025-01-01 | 학습셋 ~5% (2025-01~) |

### 진행 상황 모니터링

```bash
# 최신 진행 확인
grep "single_run market done" artifacts/batch_logs/single_run_*.log | tail -1

# 에러 확인
grep "single_run error" artifacts/batch_logs/single_run_*.log | tail -5

# DuckDB 수집 현황 (프로세스 종료 후)
python3 -c "
import duckdb
con = duckdb.connect('data/duckdb/warehouse.duckdb', read_only=True)
r = con.execute(\"SELECT model_name, COUNT(DISTINCT market_id) FROM silver_forecast_runs_hourly WHERE endpoint_kind='single_run' GROUP BY 1\").fetchall()
...
- DuckDB는 단일 writer 전용 — 수집 중에는 DuckDB에 직접 접근하지 말 것
- `--missing-only`는 DuckDB의 bronze/silver 상태를 읽어 스킵 여부 결정
- 중단 후 재시작 시 동일 명령 그대로 재실행하면 이어서 수집됨 (최대 10개 마켓 재수집 가능)
- pre-2021 마켓은 gfs 에러 없이 즉시 스킵 (min_date 필터)
- 수집 완료 후: `build-dataset` → `train-advanced` → Phase 15 autoresearch (nwp_spread 피처)
